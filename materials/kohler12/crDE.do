// Liferisk-Projekt, German data set, all Events
// kohler@wzb.eu
...
soepren partnr??, new(partnr) w(1984/2009)
soepren partz??, new(partz) w(1984/2009)
...
// "Race"
gen race:yesno = inlist(migback,2,3,4) if inrange(migback,1,4)
label variable race "Mig. background"
drop migback

// Sector
gen industry:yesno = inlist(sector,1,2,3,4,5) if inrange(sector,1,9)
label variable industry "Industrial/agricultural sector (Third=0)"
drop sector

// Whours
replace whours = . if whours < 0 | (whours > (7*18*365) & !mi(whours))
replace whours = whours/52  // whours shifted downstrem
...
// Short term illness
gen workforce = whours > 0 & !mi(whours)
gen byte ill:yesno = 0 					/// 
  if workforce                          /// Working force only
  & !inlist(wave,1990,1993)              // No data in 1999 + 1993
replace ill = 1 if workforce & inrange(illdays,5,30)
replace ill = 0 if workforce & longill == 1 & wave <= 1998
replace ill = 0 if workforce & inlist(longill,1,2) & wave > 1998
label variable ill "Short term illness"

// Long term illness
gen illlong:yesno = 0 if workforce & !mi(longill)
replace illlong = 1 if workforce & longill == 1 & wave <= 1998
replace illlong = 1 if workforce & inlist(longill,1,2) & wave > 1998
replace illlong = 1 if workforce & illdays > 30 & !mi(illdays) // As in PSID
label variable illlong "Long term illness"

// Weeks ill
gen illweeks = illdays/5 if workforce
replace illweeks = 0 if workforce & illnot==1
label variable illweeks "Weeks of illness"

foreach var of varlist ill* whours workforce {
	replace `var' = F1.`var' 
}
...
replace hhpostgoveq = hhpostgoveq * (103.9/ F1.cpi) // 2007 prices

label variable hhpostgoveq "Houshold equivalence income (post gov)"
...
replace kurzstr = "000000000000" if kurzstr == ""

// Harmonize abnormal codes of years 1996/97 
...
label variable unempmth "Month unemployed"

// Count month employed
gen empmth = 0 
forvalues mth = 1/12 {
	replace empmth = empmth + 1 	/// 
	  if substr(vollzstr, `mth' ,1) == "1" ///
	  | substr(teilzstr, `mth' ,1) == "1"  /// 
	  | substr(kurzstr, `mth' ,1) == "1" 
}
label variable empmth "Month employed"

replace unempmth = F1.unempmth
replace empmth = F1.empmth
...
label variable egph "EGP of main earner"

// Respondent
gen egpp:egp = 1 if inlist(egp,1,2)
...
label variable egpp "EGP of respondend"
drop egp
...
merge n:1 persnr using  ../../data/weights/gsoepweights  ///
  , keep(3) keepusing(weight sample) nogenerate

merge n:1 hhnr using $soep26/design 	///
...
foreach k of local K {
	...
	replace poor = hhpostgoveq < (r(r1)*.6) & !mi(hhpostgoveq) if wave==`k' 
}
...
by id (wave): gen byte eunemp = 	/// 
  inrange(unempmth,3,12) & inrange(empmth[_n-1],7,12)
label variable eunemp "Job loss"

gen eunempdata = inrange(age,25,55)
gen eunempatrisk = inrange(empmth,7,12)
...
gen eillatrisk = inrange(empmth,7,12)

// Family break-up
by id (wave), sort:  ///
  gen todpart_d = inlist(partz,0,1,2)  ///
  & partz[_n-1]==2  ///
  & inrange(todpart,1,12)

by id (wave), sort:  ///
  gen byte efambreak = 1 if mar != 5 & inrange(hhsize0to14[_n-1],1,.) ///
  & partz == 0 & inlist(partz[_n-1],1,2,3,4,9)

by id (wave):  ///
  replace efambreak = 1 if mar != 5 & inrange(hhsize0to14[_n-1],1,.) ///
  & inlist(partz,1,2) & inlist(partz[_n-1],1,2) & partnr != partnr[_n-1] 

by id (wave): ///
  replace efambreak = 1 if !todpart_d & inrange(hhsize0to14[_n-1],1,.)  ///
  & inlist(partz,1,2) & inlist(partz[_n-1],1,2) & partnr != partnr[_n-1]

replace efambreak = 0 if efambreak == .

gen efambreakdata = inrange(age,25,55)
gen efambreakatrisk = partz!=0 & inrange(hhsize0to14,1,.)
...
// Retirement
by id (wave): 						/// 
  gen atrisk = sum(age>=55 & !mi(age) & empmth>=2 & empmth<.) 

by id (wave): replace atrisk = atrisk >= 2 /// 
  if age>=55 & age<. 
drop if atrisk == 2 // 1 obs with age=61 in 2004 and age=49 in 2005

gen byte eretire = empmth <= 2 & f1.empmth <= 2 & f2.empmth<=2  ///
  if atrisk & !mi(empmth,f1.empmth,f2.empmth) & wave < 1996
replace eretire = empmth <= 2 & f1.empmth <= 2    ///
  if atrisk & !mi(empmth,f1.empmth) & wave >= 1996
...
gen eretireatrisk = inrange(empmth,2,.)
drop atrisk
...
sort id wave
keep if netto==10 | f1.netto==10

compress
label variable wave "Year"

order hhnr hnr id wave cpi netto weight psu  /// 
  men gebjahr age race ///
  todjahr hhsize* mar todpart partz partnr 	/// 
  industry egpp egph whours empmth unempmth empdur 	/// 
  edu yedu 								/// 
  ill* sick shealth ///
  poor indlabinc* hhlabinc* hhpregov* hhpostgov* debt* reserve
