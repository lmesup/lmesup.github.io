...
foreach cntry in DE US {
	...
	collapse (mean) mean=poor (sd) sd=poor (count) n=poor [aw=weight], by(t edu)
...
graph twoway 							///
  || line mean t if edu==1, sort  ///
  || line mean t if edu==2, sort 	///
  || line mean t if edu==3, sort 	///
  || if risk == "Job loss" ///
  ...
  legend(rows(1) order(1 "Low" 2 "Intermediate" 3 "High"))

graph export anpoor_edu4_unemp.eps, replace
...
graph export anpoor_edu4_ill.eps, replace
...
graph export anpoor_edu4_trennung.eps, replace
...
graph export anpoor_edu4_alter.eps, replace
