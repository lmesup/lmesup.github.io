...
foreach cntry in DE US {
	...
	foreach event in eunemp eill efambreak eretire {
		...
		foreach ctag of varlist ctag* {
			...
			// At risk of event in wave before eventyear
			gen diff = eyear-wave if wave < eyear
			by id (diff), sort: gen atrisk =  /// 
			  sum(`event'atrisk[1]==1)
			by id (diff): keep if atrisk[_N]
			drop atrisk diff
...
graph export andid_bytime.eps, replace
