...
version 8.2
	...
	graph twoway ///
	  ...
	  (dot hetero sortedcnt, ndots(30)  horizontal mcolor(black) ms(o) msize(*1.2))    ///
	  , yscale(reverse) ytitle("") ysize(3.15) xsize(4.15) ///
  	  ylabel(1(1)15 17(1)26 28(1)30, notick valuelabel angle(horizontal) gextend) ///
	  by(type, xrescale legend(off) note("") iscale(.9) graphregion(margin(zero))) ///
	  subtitle(, bfcolor(none)) 

	graph export plurality_by_country.eps, replace
