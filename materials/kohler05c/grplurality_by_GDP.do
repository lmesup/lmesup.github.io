...
version 8.2
	...
	label define EU 1 "EU-15" 2 "NMS-10" 3 "CC-3", modify
	...
	local opt `" `opt' ms(Oh O X) mcolor(black black black)"'
	local twopt `" legend(off) "'

	// Most Countries, Entropy
	// -----------------------
	
	local graph1opt `" ylabel(.7(.05).85, axis(1) grid gstyle(dot) nogextend) ytitle("Standardisierte Entropie")   "'
	...
	graph twoway ///
	  ...
	  if iso3166_2 == "LU"  ///
	  , ytick(.7(.05).85, axis(2) grid gstyle(dot)) xscale(range(45000 45720) axis(1)) xtick(45360, axis(2))   ///
	...
	// Legend
	scatter entropy_s? gdppcap1, ms(Oh O X) mcolor(black black black) ///
	  legend(lab(1 "EU-15") lab(2 "NMS-10") lab(3 "CC-3") order(3 2 1) rows(1)) ///
   ...
   _gm_edit .leg.ystretch.set fixed
	...
	// Combine them
	graph combine g1 g2 leg , cols(1) imargin(tiny) xsize(2.76) ysize(4.14) iscale(*1.2)
	graph export plurality_by_GDP.eps, replace
