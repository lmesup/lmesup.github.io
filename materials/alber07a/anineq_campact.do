...
* Author: lenarz@wzb.eu -> diff_campact.do
...
use ///
 persid iso3166 dataset campact hhinc edu emp weight ///
 using cses, clear
capture append using ess02  ///
 , keep(persid iso3166 dataset campact hhinc edu emp weight  )
...
quietly foreach dataset of local D {
  ...
  foreach k of local K {
    count if !mi(campact) & !mi(hhinc5) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg campact hhinc2-hhinc5 if iso3166 == "`k'" & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("hhinc") (_b[hhinc5])
    }
    count if !mi(campact) & !mi(edu3) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg campact edu2 edu3 if iso3166 == "`k'"  & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("edu") (_b[edu3])
    }
    count if !mi(campact) & !mi(emp1) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
        reg campact emp1 emp3-emp5 if iso3166 == "`k'" & dataset == "`dataset'"
...
foreach strat of local K {
  ...
  graph twoway ///
     ...
     || , ylabel(`I', valuelabel angle(0)) ytitle("")            ///
          xtitle("Inequality of participating in election campaign (`strat')") ///
          xsize(4) ysize(6) legend(off)
  graph export anineq_campact_`strat'.eps, replace preview(on)
