...
version 10
   ...
   label define dim 1 "Einkommen (5. - 1. Quintil)" 2 "Bildung (Tertiär - Primär)" 
	...
	graph twoway ///
      ...
      by(dim, legend(off) note("") ///
  title(`"Ungleichheit der Wahlbeteiligung in den Dimensionen"' ///
        `"Haushaltseinkommen und Bildung"', size(medlarge)))            ///
	  ysize(5) xsize(4.5) 

	graph export ../figure_albergeb3DE.eps, replace preview(on) 
 ...
 graph twoway ///
   ...
   _gm_edit .leg.ystretch.set fixed
	...
	graph combine g1 leg, cols(1)  ///
     note("") ///
     title(`"Ungleichheit der Wahlbeteiligung in Ländern mit"' 	///
           `"unterschiedlichen Sozialausgabe- und Staatseinnahmequoten"'             ///
                , size(medlarge))            ///
	  ysize(4) xsize(4.5) 
	graph export ../figure_albergeb4DE.eps, replace preview(on) 
