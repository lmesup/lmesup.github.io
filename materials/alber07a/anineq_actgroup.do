* Difference in actgroups with politician
* Author: lenarz@wzb.eu -> diff_actgroup.do
...
use ///
 persid iso3166 dataset actgroup hhinc edu emp weight ///
 using cses, clear
append using ess02  ///
 , keep(persid iso3166 dataset actgroup hhinc edu emp weight  )
append using ess04  ///
 , keep(persid iso3166 dataset actgroup hhinc edu emp weight  )
append using issp04 ///
 , keep(persid iso3166 dataset actgroup hhinc edu emp weight  )
...
quietly foreach dataset of local D {
  ...
  foreach k of local K {
    count if !mi(actgroup) & !mi(hhinc5) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg actgroup hhinc2-hhinc5 if iso3166 == "`k'" & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("hhinc") (_b[hhinc5])
    }
    count if !mi(actgroup) & !mi(edu3) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg actgroup edu2 edu3 if iso3166 == "`k'"  & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("edu") (_b[edu3])
    }
    count if !mi(actgroup) & !mi(emp1) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
        reg actgroup emp1 emp3-emp5 if iso3166 == "`k'" & dataset == "`dataset'"
...
foreach strat of local K {
  ...
  graph twoway ///
     ...
     || , ylabel(`I', valuelabel angle(0)) ytitle("")            ///
          xtitle("Inequality of participating in activity group (`strat')") ///
          xsize(4) ysize(6) legend(off)
  graph export anineq_actgroup_`strat'.eps, replace preview(on)
