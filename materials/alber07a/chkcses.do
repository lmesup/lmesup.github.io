	...
	version 9.2
	capture log close
	log using chkcses, replace
	use cses, clear
	
	// Constant Data Range over datasets and countries -> Note 1
	chkmin intmen men edu-hhinc church-econchange, by(dataset iso3166)
	chkmax intmen men edu-hhinc church-econchange, by(dataset iso3166)
	...
	chklabobs fairelect partcare partneces polknow econdevel econchange ///
	  if dataset == "CSES-MODULE-1" , by(iso3166) 

	chklabobs persother campact protest actgroup humrights corruption ///
	  if dataset == "CSES-MODULE-2", by(iso3166) 

	chklabobs men age edu emp mar hhinc church rel denom rural ///
	  unionmemb unionmembhh ///
	  voter pi pii leftright democsat ///
	  powmatters votematters polinform , by(dataset iso3166)
	// Note: No protestants accepted in Portugal and Espania
	
	// Distributions 

	encode dataset, gen(data)
	chkfreq men data, over(iso3166) 
	chkfreq edu data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3))) // Note 0
	chkfreq emp data, over(iso3166) legend(cols(1)) byopt(legend(off))
	chkfreq mar data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq hhinc data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq church data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq rel data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq denom data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq rural data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq unionmemb data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq unionmembhh data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq voter data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq contact data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3))) // Note 1
	chkfreq persother, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3))) 
	chkfreq campact, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3))) 
	chkfreq protest, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3))) 
	chkfreq actgroup, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3))) 
	chkfreq pi data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq pii data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq democsat data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq powmatters data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq polinform data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	chkfreq econdevel data, over(iso3166) legend(cols(1)) byopt(cols(1) legend(pos(3)))
	log close
