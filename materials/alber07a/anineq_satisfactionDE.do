...
graph twoway ///
   ...
   || if natfam < 5                                               ///
      ...
      xtitle("Differenz der Wahlbeteiligung" "(Zufriedene minus Unzufriedene)") ///
	  xlabel(-.2(.2).4) xtick(-.2(.1).4) xmtick(-.15(.1).35)     ///
      by(strat, legend(off) ///
      note("Quellen: CSES I, CSES II, ESS '02, ESS '04, ISSP 2004", span)    ///
      title(`"Grafik 6: "', span ring(2) pos(11) justification(left) size(medlarge)) ///
      subtitle(`"Differenz der Wahlbeteiligung unter"'                                 ///
               `"politisch Zufriedenen und "' ///
               `"Unzufriedenen"', margin(l+18 b+3)             ///
                span ring(2) pos(11) justification(left) size(medlarge)))            ///
	  ysize(5.5) xsize(4.5) 

	graph export ../figure6DE.eps, replace preview(on) 
