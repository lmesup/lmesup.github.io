	// European Social Survey from 2002 
	// -------------------------------------------------
	// (Subset of European Social Survey from 2002)
...
version 9
	...
	gen edu:edu = 1 if inlist(edulvl,0,1,2)				// REKODIERUNG CHECKEN!
	replace edu = 2 if inlist(edulvl,3,4)
	...
	gen emp:emp = .
	replace emp = 5 if dngoth == 1 | dsbld == 1 | cmsrv == 1 | hswrk ==1
	replace emp = 2 if uempla == 1 | uempli == 1
	replace emp = 4 if rtrd == 1
	replace emp = 1 if pdwrk == 1
	replace emp = 3 if edctn == 1 & age <31 
	...
	*label variable rural "Living in rural area/village" 
	...
	egen donate = anymatch(dntmny sptcdm hmnodm epaodm prtydm), v(1)
	egen mis = rmiss(dntmny sptcdm hmnodm epaodm prtydm)
	replace donate = . if mis>0
	...
	keep dataset dataset_version persid weight intdate mode iso3166 ///
	age men edu mar unionmemb emp hhinc hhsize church rel ///
	denom voter petition protest actgroup contact donate democsat ///
	leftright polint 	
	
	drop if age < 18

	label data "Subset of ESS 2002"
	save ess02, replace
