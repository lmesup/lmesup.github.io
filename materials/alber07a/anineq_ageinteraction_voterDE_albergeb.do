...
graph twoway ///
  ...
  by(strat,                                                   ///
     note("") ///
  subtitle(`"Ungleichheit der Wahlbeteiligung der Rentner-"'  /// 
   `"bevölkerung und der  jüngeren Generation"', size(medlarge)))            ///
  ysize(4.5) xsize(4)                                                               ///
  legend(order(2 "30-64" 1 "65 und älter"))                                         ///
  xtitle("Differenz der Wahlbeteiligung" "(Statushöhere minus Statusniedere)")      

graph export ../figure_albergeb5DE.eps, replace preview(on) 
