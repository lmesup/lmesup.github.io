...
version 9.2
	...
	sort natfam iso3166
	listtex iso3166 surveys voter_hhinc* voter_edu* using anineq_voter.txt ///
	...
	// Graph titles
	replace strat = "5th vs 1st Income Quint." if strat == "hhinc"
	replace strat = "Tertiary vs. Primary Educ." if strat == "edu"
	...
	graph twoway ///
      ...
      xtitle("Turnout among higher strata minus turnout among lower strata")  ///
	   xlabel(-.2(.2).4) xtick(-.2(.1).4) xmtick(-.15(.1).35)       ///
      by(strat, legend(off) ///
      note("Source: CSES I, CSES II, ISSP '04, ISSP '02, ESS '02, ESS '04, EQLS '03", span) ///
	   title("Figure 3" "Inequality of electoral participation by country")) ///
	  ysize(6.7) xsize(10) 
	
	graph export ../figure3.eps, replace preview(on) 
	...
	listtex iso3166 order fund socbrut socnet01 taxrev04 pens04  ///
	  using anineq_voter_inclusive.txt     ///
	  , replace rstyle(tabdelim)
	restore
  ...
  graph combine socbrut socnet01 taxrev04 pens04, ///
    l1title("Inequality of electoral participation") ///
 ...
 graph twoway ///
	 || scatter meanb socbrut if natfam == 1            ///
	  ...
	  , ms(S) mlcolor(black) mfcolor(white)              ///
	  legend(rows(1) order(1 "US" 2 "EU-15" 4 "Transf." 3 "Other")) ///
   ...
   _gm_edit .leg.ystretch.set fixed
	...
	graph combine g1 leg, cols(1) xsize(10) ysize(6.5) ///
    note("Source: CSES, ISSP, ESS, EQLS. (Inequality of particitpation); OECD, Eurostat (Inclusiveness)", span) ///
    title("Figure 4" "Inequality of electoral participation by inclusiveness") 

	graph export ../figure4.eps, replace preview(on) 
