...
by iso3166 strat: replace meanbold = meanbold[_N]
	...
	replace strat = "Education" if strat == "edu"
	replace strat = "Income" if strat == "hhinc"
...
levelsof axis, local(I)
	...
	graph twoway ///
	  ...
	  || dot meanb axis`strat'                                    ///
	  , horizontal msymbol(O) mcolor(black)                       ///
	  || , ylabel(`I', valuelabel angle(0)) ytitle("")            ///
	  by(strat, ///
      note("Source: CSES I, CSES II, ISSP '04, ISSP '02, ESS '02, ESS '04, EQLS '03", span) ///
	  title("Figure 5" "Ineq. of electoral participation by age")) ///
	  ysize(9) xsize(6.5)                                         ///
	  legend(order(2 "30-64" 1 "65 and older"))                   ///
      xtitle("Turnout among higher strata minus turnout among lower strata")  ///

	graph export ../figure5.eps, replace preview(on) 
