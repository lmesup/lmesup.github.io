...
graph twoway ///
  ...
  || sc axis meanb if meanb<meanbold, mlab(meanb) mlabpos(9) ms(i) ///
  || sc axis meanb if meanb>meanbold, mlab(meanb) mlabpos(3) ms(i) ///
  || sc axis meanbold if meanb<meanbold, mlab(meanbold) mlabpos(3) ms(i) ///
  || sc axis meanbold if meanb>meanbold, mlab(meanbold) mlabpos(9) ms(i) ///
  || , ylabel(`I', valuelabel angle(0) gstyle(dot) labsize(*.8)) ytitle("") ///
  by(strat,                                                   ///
     note("") ///
     title(`"Ungleichheit der Wahlbeteiligung der Rentner-"'                             ///
              `"bevölkerung und der jüngeren Generation"',                          ///
               span ring(2) pos(11) justification(left) size(medlarge)))            ///
  ysize(5.5) xsize(5)                                                               ///
  legend(order(2 "30-64" 1 "65 und älter"))                                         ///
  xtitle("Wahlbeteiligung Statushöhere minus Statusniedere")      ///
  xscale(range(-.1 .6))
graph export 11.eps, replace 
!epstopdf --outfile=anineq_ageinteraction_voterDE_withnumbers.pdf 11.eps 

by axis strat, sort: keep if _n==1
gsort - axis
egen ctrnames = iso3166(iso3166), o(codes)
format meanb meanbold %8.6f

outsheet iso3166 meanb meanbold ctrnames using anineq_ageinteraction_voterDE_withnumbers_Bildung.tsv if strat=="Bildung", replace noquote

outsheet iso3166 meanb meanbold ctrnames using anineq_ageinteraction_voterDE_withnumbers_Einkommen.tsv if strat=="Einkommen", replace noquote
