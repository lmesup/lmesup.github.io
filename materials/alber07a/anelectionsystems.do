...
version 9.2
	...
	assert _merge == 3
	drop _merge
	merge iso3166 using `system2', nokeep sort
	assert _merge == 3
	drop _merge
	merge iso3166 using `manifesto', nokeep sort
	assert _merge == 3
	drop _merge
	...
	// Define sort order
	egen axis = axis(natfam turnout) if natfam < 5
	sort axis

	// Produce table
	listtex iso3166 sdate turnout weekend branch compvote comp_reg elecsys ///
	  compet multip ///
	  using anelectionsystems.txt, replace rstyle(tabdelim)
	...
	by natfam, sort: reg turnout multip
	reg turnout multip if natfam==2
...
sort sim1
list eldate  iso3166 sim1 turnout
...
sort sim2
list eldate  iso3166 sim2 turnout


gen exec = branch == "Exec."
gen comp = compvote == "Yes"
reg turnout weekend exec comp comp_reg proportional compet multip ///
 if natfam == 2
