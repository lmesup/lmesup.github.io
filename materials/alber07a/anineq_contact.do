* Difference in contacts with politician
* Author: lenarz@wzb.eu -> diff_contact.do
...
use ///
 persid iso3166 dataset contact hhinc edu emp weight ///
 using cses, clear
append using ess02  ///
 , keep(persid iso3166 dataset contact hhinc edu emp weight  )
append using ess04  ///
 , keep(persid iso3166 dataset contact hhinc edu emp weight  )
append using issp04 ///
 , keep(persid iso3166 dataset contact hhinc edu emp weight  )
...
quietly foreach dataset of local D {
  ...
  foreach k of local K {
    count if !mi(contact) & !mi(hhinc5) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg contact hhinc2-hhinc5 if iso3166 == "`k'" & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("hhinc") (_b[hhinc5])
    }
    count if !mi(contact) & !mi(edu3) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg contact edu2 edu3 if iso3166 == "`k'"  & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("edu") (_b[edu3])
    }
    count if !mi(contact) & !mi(emp1) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
        reg contact emp1 emp3-emp5 if iso3166 == "`k'" & dataset == "`dataset'"
...
foreach strat of local K {
  ...
  graph twoway ///
     ...
     || , ylabel(`I', valuelabel angle(0)) ytitle("")            ///
          xtitle("Inequality of contacts with politicians (`strat')") ///
          xsize(4) ysize(6) legend(off)
  graph export anineq_contact_`strat'.eps, replace preview(on)
