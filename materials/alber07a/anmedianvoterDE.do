...
version 9.2
   ...
   drop if iso3166=="TR"
	...
	graph twoway ///
      ...
      xtitle("Durchschnittl. Einkommensquintil")             ///
	  xlabel(2(1)4) xtick(2(1)4) xmtick(2.5(1)3.5)            ///
      legend(off)                                            ///
      note("Quellen: CSES I, CSES II, ISSP '04, ISSP '02, ESS '02, ESS '04, EQLS '03", span) ///
     title(`"Grafik 2: "', span ring(2) pos(11) justification(left) size(medlarge))  ///
      subtitle(`"Durchschnittsposition der Wähler in"'                  ///
               `"der nationalen Einkommensverteilung"', margin(l+17 b+3) ///
                span ring(2) pos(11) justification(left) size(medlarge))             ///
	  ysize(6) xsize(4) 


	graph export ../figure2DE.eps, replace preview(on)
	exit
