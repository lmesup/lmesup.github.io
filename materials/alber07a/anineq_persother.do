...
* Author: lenarz@wzb.eu -> diff_persother.do
...
use ///
 persid iso3166 dataset persother hhinc edu emp weight ///
 using cses, clear
capture append using ess02  ///
 , keep(persid iso3166 dataset persother hhinc edu emp weight  )
capture append using ess04  ///
 , keep(persid iso3166 dataset persother hhinc edu emp weight  )
capture append using issp04 ///
 , keep(persid iso3166 dataset persother hhinc edu emp weight  )
capture append using eqls03 ///
 , keep(persid iso3166 dataset persother hhinc edu emp weight  )
...
quietly foreach dataset of local D {
  ...
  foreach k of local K {
    count if !mi(persother) & !mi(hhinc5) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg persother hhinc2-hhinc5 if iso3166 == "`k'" & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("hhinc") (_b[hhinc5])
    }
    count if !mi(persother) & !mi(edu3) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
      reg persother edu2 edu3 if iso3166 == "`k'"  & dataset == "`dataset'"
      post coefs ("`k'") ("`dataset'") ("edu") (_b[edu3])
    }
    count if !mi(persother) & !mi(emp1) & dataset == "`dataset'" & iso3166 == "`k'"
    if r(N) > 300 {
        reg persother emp1 emp3-emp5 if iso3166 == "`k'" & dataset == "`dataset'"
...
foreach strat of local K {
  ...
  graph twoway ///
     ...
     || , ylabel(`I', valuelabel angle(0)) ytitle("")            ///
          xtitle("Inequality of persuating others (`strat')") ///
          xsize(4) ysize(6) legend(off)
  graph export anineq_persother_`strat'.eps, replace preview(on)
