...
version 9
	...
	|| sc axis samplex5, ms(Th) mlc(black) mfc(white)   /// 
	  ...
	  title(Sample method, pos(12) box bexpand fcolor(gs12) ) nodraw name(g1, replace) ///
	...
	// Legend
	tw sc substx1 substx2 substx3 substx4 substx5 axis ///
	  , legend(order(1 "EB" 2 "EQLS" 3 "ESS" 4 "EVS" 5 "ISSP") rows(1))  ///
	...
	graph combine combined leg, cols(1) ///
	  title("Figure 1: Components of the sampling process") ///
	  subtitle("By country and survey program") ///
	  note(Own calculations. Do-File: ansample02_1.do)

	graph export ansample02_1.eps, replace preview(on)
	...
	listtex survey bestsamp bestsubst bestbac index using ansample02_1.tex ///
	  , rstyle(tabular) replace missnum("n.a.")
