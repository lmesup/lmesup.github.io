...
version 9
	...
	use s_cntry emplfem2 seatswom hdirank using $dublin/eqls_4, clear
	...
	foreach k of local K {
		...
		gen empiso`i' = emp * (iso3166_2 == "`k'")
		lab var womeniso`i' `"`k'"'
		lab var womeniso`i++' `"`k'"'
	}

	clogit selected women emp age if !city, group(id)
	local overall_women = _b[women]
	local overall_emp = _b[women]
	
	clogit selected womeniso* empiso* age if city, group(id)
	matrix b = e(b)'
	svmat b, names(bcity)

	clogit selected womeniso* empiso* age if !city, group(id)
	matrix b = e(b)'
	svmat b, names(brural)
	...
	gen group = _n>28
	gen iso = _n in 1/28
	replace iso = _n - 28 in 29/56
	reshape wide bcity1 brural1, i(iso) j(group)
	sort iso
	merge using `patr'

	tw  ///
	  sc bcity11 brural11  hdi    ///
	  || lowess bcity11 hdi ///
	  || lowess brural11 hdi ///
	  , yline(0)       ///
	...
	tw  ///
	  sc bcity10 brural10  emplfem    ///
	  || lowess bcity10 emplfem ///
	  || lowess brural10 emplfem ///
	  , yline(0)       ///
	  ytitle(Coef. Conditonal Logit) ylabel(, grid)
