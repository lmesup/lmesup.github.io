 * Analyses for CREST/ISEE, 24 Apr 2009
* kohler@wzb.eu
...
foreach var of varlist men age emp1 edu3 urban hqual roomspers ///
  vol contacts rel health illness {
	...
	graph export anparis_`var'.eps, replace
}
...
graph twoway ///
  ...
  ysize(6) xsize(4)
graph export anparis_class.eps, replace
...
graph twoway ///
  ...
  pos(6) col(4) size(small)) ///
  ysize(6) xsize(4)
graph export anparis_mar.eps, replace
...
ren edu3 Education
ren rel Church
ren contacts Contacts
ren hqual Housing 
ren roomspers Flatsize
ren vol Voluntary
ren mar1 Married
ren class1 Service
ren class4 Worker

biplot8 Health Employment Education Church Contacts Housing ///
  Flatsize Married Service Worker ///
  , subpop(eu, mlab(iso3166 iso3166 iso3166) mlabsize(*.6..)  /// 
  mlabcolor(black..) ms(O..) ///
  mlcolor(black..) mfcolor(black gs8 white))  ///
  scheme(s1mono) legend(off)
graph export anparis_biplot.eps, replace 
...
esttab using anparis_step1.tex, replace ///
...
// The Models
forv i = 1/5 {
	statsby _b _se, by(iso3166) saving(anparis_step2_m`i', replace): ///
	  reg lsat  `m`i''
}
...
merge iso3166 using anparis_step2_m1, sort
...
restore, preserve
by iso3166, sort: keep if _n==1
merge iso3166 using anparis_step2_m2, sort
...
restore , preserve
by iso3166, sort: keep if _n==1
merge iso3166 using anparis_step2_m3, sort
...
restore , preserve
by iso3166, sort: keep if _n==1
merge iso3166 using anparis_step2_m4, sort
...
restore, preserve
by iso3166, sort: keep if _n==1
merge iso3166 using anparis_step2_m5, sort
...
graph export anparis_step2_askriptiv.eps, replace

graph display step2_illness
graph export anparis_step2_illness.eps, replace

grc1leg step2_edu2 step2_edu3, rows(1) ycommon
graph export anparis_step2_edu.eps, replace

grc1leg step2_emp3 step2_emp4, rows(1) ycommon
graph export anparis_step2_emp.eps, replace

grc1leg step2_class4 step2_class5, rows(1) ycommon
graph export anparis_step2_class.eps, replace

grc1leg step2_inc2 step2_inc4, rows(1) ycommon
graph export anparis_step2_inc.eps, replace

grc1leg step2_urban step2_roomspers step2_hqual, rows(1) ycommon
graph export anparis_step2_housing.eps, replace

grc1leg step2_mar2 step2_mar4, rows(1) ycommon
graph export anparis_step2_mar.eps, replace

grc1leg step2_vol step2_contacts step2_rel, rows(1)
graph export anparis_step2_sozbez.eps, replace
...
esttab using anparis_step3.tex, replace ///
...
statsby _b _se, by(iso3166) saving(anparis_replication_m4, replace): ///
  reg lsat men age age2 edu2 edu3 emp2-emp5 inc2-inc4

statsby _b _se, by(iso3166) saving(anparis_replication_m5, replace): ///
...
keep iso3166 hdi
merge iso3166 using anparis_replication_m4, sort
keep if _merge==3
...
keep iso3166 hdi
merge iso3166 using anparis_replication_m5, sort
keep if _merge==3
...
graph export anparis_replication_inc.eps, replace

graph combine replication_mar3 replication_mar4, rows(1) ycommon
graph export anparis_replication_mar.eps, replace
