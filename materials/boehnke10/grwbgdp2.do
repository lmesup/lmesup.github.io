* Well-Being (Life-Satisfaction) by GDP-Rank, Country-Specific over Time
...
version 9
	...
	// Calculate Ranks only for EU
	by iso3166_2, sort: keep if _N >= 30
	by year (GDP iso3166_2), sort: gen gdprank = _N - _n + 1
	
	// Group countries according to correlations
	separate satislfe, by(iso3166_2) veryshortlabel
	gen group:group = .
	foreach var of varlist satislfe? {
		reg `var' gdprank
		replace group = 1 if (abs(_b[gdprank]) > (2 * _se[gdprank])) & _b[gdprank] > 0 & e(sample)
		replace group = 2 if  abs(_b[gdprank]) < (2 * _se[gdprank]) & e(sample)
		replace group = 3 if (abs(_b[gdprank]) > (2 * _se[gdprank])) & _b[gdprank] < 0 & e(sample)
	}
	...
	drop if satislfe >= . | gdprank >= .
	by iso3166_2 (gdprank), sort: gen mlaby = satislfe if _n==1 &  ///
	  ( iso3166_2 == "IE"  ///
	  | iso3166_2 == "GR"  ///
	  | iso3166_2 == "ES"  ///
	  | iso3166_2 == "FI"  ///
	...
	by iso3166_2 (gdprank), sort: replace mlaby = satislfe if _n==_N & mlaby[1] >= .
	gen mlabvpos = 3
	...
	tw ///
	  || line satislfe? satislfe? gdprank, lcolor(black..) ///
