* Graphiken Model 3 (zeitverzögert, permanent)
version 8.0
...
ren bleft10 bleft0
ren bkons10 bkons0
gen bleft1 = bleft0 + 3 * bleft11
gen bkons1 = bkons0 + 3 * bkons11
drop bleft11 bkons11
...
foreach piece in arbsel selarb parspd parcdu {
  ...
  * Left-Model, No political interest
  gen `piece'fl0 = 0
 replace `piece'fl0 = `piece'fl0 + bleft0[_N] * (1 + ln(t-4)) if t>=5 & t ~= .
  * Left-Model, Political interest
  gen `piece'fl1 = 0
  replace `piece'fl1 = `piece'fl1 + bleft1[_N] * (1 + ln(t-4)) if t>=5 & t ~= .
  * Between effects
  gen `piece'bl = bleft_be[_N] if t ~= .
  * Kons-Model, No political interest
  gen `piece'fc0 = 0
  replace `piece'fc0 = `piece'fc0 + bkons0[_N] * (1 + ln(t-4)) if t>=5 & t ~= .
  * Kons-Model, Political interest
  gen `piece'fc1 = 0
  replace `piece'fc1 = `piece'fc1 + bkons1[_N] * (1 + ln(t-4)) if t>=5 & t ~= .
  * Between effects
...
sum arbselfl0 if t == 10
local noint = r(mean)
sum arbselfl1 if t == 10
local int = r(mean)
...
graph combine gleft gcons, rows(2) xcommon ycommon note("Do File: grmod3.do")
graph export figure6.wmf, replace
...
sum parspdfl0 if t == 10
local noint = r(mean)
sum parspdfl1 if t == 10
local int = r(mean)
...
sum parcdufl0 if t == 10
local noint = r(mean)
sum parcdufl1 if t == 10
local int = r(mean)
...
sum parspdfc0 if t == 10
local noint = r(mean)
sum parspdfc1 if t == 10
local int = r(mean)
...
graph twoway line parspdfc* t, ///
       ...
       text(`noint' 10 "low pol. interest", placement(nw)) ///
       text(`int' 10 "high pol. interest", placement(nw)) 
sum parcdufc0 if t == 10
local noint = r(mean)
sum parcdufc1 if t == 10
local int = r(mean)
...
graph combine gleft gcons, rows(2) xcommon ycommon note("Do File: grmod3.do") 
graph export figure7.wmf, replace
