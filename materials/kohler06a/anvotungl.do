...
version 8.2
	...
	sort s_cntry
	merge s_cntry using  anvotstrat, nokeep
	drop _merge
	sort s_cntry
	merge s_cntry using anvotzuf, nokeep
	drop _merge
	sort s_cntry
	...
	graph twoway ///
	  ...
	  ytitle("Soziostrukturelle Ungleichheit d. Wahlbeteiligung" "standardisiert") ///
	  xtitle("Ungleichheit d. Wahlbeteiligung hinsichlich Zufriedenheit" "standardisiert") ///
	  xlab(, grid)  ylab(, grid) ///
	  yline(0) xline(0) ///
	  note("Symbolgröße proportional zur Wahlabstinenz", span)
	graph export anvotungl.eps, replace
	...
	file open results using anvotungl.tex, write text replace
