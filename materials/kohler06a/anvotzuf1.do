	// Logit-Models Turnout on satisfaction  * country

version 8.2
	clear
	set more off
	set matsize 200
	set scheme s1mono
	capture log close
	log using anvotzuf1, replace
	
	use s_cntry hh2a hh2b hhinc4 q24a q25 q29* q30*  q31 q46 q54* q56* hhstat emplstat wcountry ///
	  using $dublin/eqls_4, clear

	// Voter
	// ----

	// Nur Wahlberechtigte
	drop if q25 == 3 | q25 >= .
	gen voter = q25==1 

	// Merge Nice Country-Names
	// ------------------------

	sort s_cntry
	merge s_cntry using isocntry
	drop _merge

	// Rectangularize Data
	// -------------------

	// I'll keep Missings of Categorical Data
	replace emplstat = 7 if emplstat == .
	replace hhstat = 7 if hhstat == .

	// Listwise Deletion
	mark touse
	markout touse s_cntry hh2a hh2b hhinc4 q24a q25 q31 q46 hhstat emplstat
	keep if touse


	// Label according to the Sort-Order
	// ---------------------------------

	preserve
	// Country-Dummies (in sort order for nice tables)
	by ctrde, sort: gen fraction = sum(voter*wcountry)/sum(wcountry)
	by ctrde, sort: replace fraction = fraction[_N]
	bysort eu fraction: keep if  _n==1
	gen ctrsort:ctrsort = (eu-1) + _n
	count
	local n = r(N)
	forv i = 1/`n' {
		local label = ctrde[`i']
		local nr = ctrsort[`i']
		label define ctrsort `nr' "`label'", modify
	}
	keep ctrde ctrsort
	sort ctrde
	tempfile ctrsort
	save `ctrsort'
	restore

	sort ctrde
	merge ctrde using `ctrsort'
	drop _merge

	levels ctrsort, local(K)
	foreach k of local K {
		gen ctrsort`k' = ctrsort == `k'
		local label: label (ctrsort) `k'
		label variable ctrsort`k' "`label'"
	}


	// Recoding
	// --------
	
	// Gender
	gen men = hh2a == 1
	label variable men "Mann"
	
	// Centered Age/Age-squared
	sum hh2b
	gen age = hh2b - r(mean)
	gen age2 = age^2
	label var age "Alter"
	label var age2 "Alter (quadriert)"

	// Income
	gen income = log(hhinc4)
	sum income
	replace income = income - r(mean)
	label var income "HH-Äquiv.-Eink. (in PPS)"

	// Education
	egen edu = xtile(q46), p(10(10)90) by(s_cntry)
	sum edu, meanonly
	replace edu = edu - r(mean)
	label var edu "Bildung (Dezile)"

	// Occupation (Household)
	gen occ1 = hhstat==1
	label var occ1 "Dienstklasse"
	gen occ2 = hhstat==2
	label var occ2 "Andere Nicht Manuelle"
	gen occ3 = hhstat==3
	label var occ3 "Selbständige "
	gen occ4 = hhstat==4 | hhstat==5
	label var occ4 "Arbeiter"
	gen occ5 = hhstat == 6 | hhstat == 7
	label var occ5 "Sonstige/Missing"
	
	// Employment (Respondend)
	gen emp1 = emplstat==1
	label var emp1 "Erwerbstaetige"
	gen emp2 = emplstat==2
	label var emp2 "Hausfrau/mann"
	gen emp3 = emplstat==3
	label var emp3 "Arbeitslos"
	gen emp4 = emplstat==4
	label var emp4 "Rentner/Pensionäre"
	gen emp5 = emplstat== 5 | emplstat==6 | emplstat == 7
	label var emp5 "Sonstige/Missing"
	
	// Group-Participation
	gen group = q24 == 1
	label var group "Mitarbeit in Gruppe"

	// Life Satisfaction
	gen lsat = q31
	label var lsat "Allg. Lebenszufriedenheit"

	// Tensions
	egen clev = rsum(q29a q29b)
	replace clev = . if clev == 0
	
	// Anomie
	gen anom = q30b + q30e
	sum anom, meanonly
	replace anom = r(max) + 1 - anom

	// Quality of Public Services
	egen pubqual = rsum(q54*)
	replace pubqual = . if pubqual == 0

	// Quality of the Environment
	egen envqual = rsum(q56*)
	replace envqual = . if envqual == 0
	sum envqual, meanonly       // Mirror, to have high values hight "satisfaction"
	replace envqual = r(max) + 1 - envqual

	
	// Life Satisfaction-Model With Control
	// ------------------------------------

	// Interactions
	foreach var of varlist ctrsort2-ctrsort30 {
		gen `var'lsat = `var' * lsat
		local label: variable label `var'
		label variable `var'lsat "`label' * lsat"
	}

	// Model
	logit voter ctrsort2-ctrsort30 men age age2 edu ///
	  income occ2-occ5 emp2-emp5  ///
	  group lsat ctrsort2lsat - ctrsort30lsat [pw=wcountry]
	estimates store lsatwc

	// Predicted values
	gen Llowlsatwc   = _b[_cons] + _b[lsat] if ctrsort==1
	gen Lhighlsatwc  = _b[_cons] + _b[lsat]*10 if ctrsort==1
	forv i = 2/30 {
		capture replace Llowlsatwc  = _b[_cons] + _b[ctrsort`i'] + _b[lsat] + _b[ctrsort`i'lsat] if ctrsort==`i'
		capture replace Lhighlsatwc = _b[_cons] + _b[ctrsort`i'] + _b[lsat]*10 + _b[ctrsort`i'lsat]*10 if ctrsort==`i'
	}

	// Life Satisfaction-Model Without Control
	// --------------------------------------

	// Model
	logit voter ctrsort2-ctrsort30 lsat ctrsort2lsat - ctrsort30lsat [pw=wcountry]
	estimates store lsatwoc

	// Predicted values
	gen Llowlsatwoc   = _b[_cons] + _b[lsat] if ctrsort==1
	gen Lhighlsatwoc  = _b[_cons] + _b[lsat]*10 if ctrsort==1
	forv i = 2/30 {
		capture replace Llowlsatwoc  = _b[_cons] + _b[ctrsort`i'] + _b[lsat] if ctrsort==`i'
		capture replace Lhighlsatwoc = _b[_cons] + _b[ctrsort`i'] + _b[lsat]*10 + _b[ctrsort`i'lsat]*10 if ctrsort==`i'
	}


	drop  ctrsort2lsat-ctrsort30lsat


	// Public Services-Model With Control
	// ----------------------------------

	// Interactions
	foreach var of varlist ctrsort2-ctrsort30 {
		gen `var'pubqual = `var' * pubqual
		local label: variable label `var'
		label variable `var'pubqual "`label' * pubqual"
	}

	// Model
	logit voter ctrsort2-ctrsort30 men age age2 edu ///
	  income occ2-occ5 emp2-emp5  ///
	  group pubqual ctrsort2pubqual - ctrsort30pubqual [pw=wcountry]
	estimates store pubqualwc

	// Predicted values
	gen Llowpubqualwc   = _b[_cons] + _b[pubqual] if ctrsort==1
	gen Lhighpubqualwc  = _b[_cons] + _b[pubqual]*50 if ctrsort==1
	forv i = 2/30 {
		capture replace Llowpubqualwc  = _b[_cons] + _b[ctrsort`i'] + _b[pubqual] + _b[ctrsort`i'pubqual] if ctrsort==`i'
		capture replace Lhighpubqualwc = _b[_cons] + _b[ctrsort`i'] + _b[pubqual]*50 + _b[ctrsort`i'pubqual]*50 if ctrsort==`i'
	}

	// Public Services Model Without Control
	// --------------------------------------

	// Model
	logit voter ctrsort2-ctrsort30 pubqual ctrsort2pubqual - ctrsort30pubqual [pw=wcountry]
	estimates store lsatwoc


	// Predicted values
	gen Llowpubqualwoc   = _b[_cons] + _b[pubqual] if ctrsort==1
	gen Lhighpubqualwoc  = _b[_cons] + _b[pubqual]*50 if ctrsort==1
	forv i = 2/30 {
		capture replace Llowpubqualwoc  = _b[_cons] + _b[ctrsort`i'] + _b[pubqual] + _b[ctrsort`i'pubqual] if ctrsort==`i'
		capture replace Lhighpubqualwoc = _b[_cons] + _b[ctrsort`i'] + _b[pubqual]*50 + _b[ctrsort`i'pubqual]*50 if ctrsort==`i'
	}

	drop  ctrsort2pubqual-ctrsort30pubqual


	// Tensions-Modell with Control
	// ----------------------------

	// Interactions
	foreach var of varlist ctrsort2-ctrsort30 {
		gen `var'clev = `var' * clev
		local label: variable label `var'
		label variable `var'clev "`label' * clev"
	}

	// Model
	logit voter ctrsort2-ctrsort30 men age age2 edu ///
	  income occ2-occ5 emp2-emp5  ///
	  group clev ctrsort2clev - ctrsort30clev [pw=wcountry]
	estimates store clevwc

	// Predicted values
	gen Llowclevwc   = _b[_cons] + _b[clev] if ctrsort==1
	gen Lhighclevwc  = _b[_cons] + _b[clev]*6 if ctrsort==1
	forv i = 2/30 {
		capture replace Llowclevwc  = _b[_cons] + _b[ctrsort`i'] + _b[clev] + _b[ctrsort`i'clev] if ctrsort==`i'
		capture replace Lhighclevwc = _b[_cons] + _b[ctrsort`i'] + _b[clev]*6 + _b[ctrsort`i'clev]*6 if ctrsort==`i'
	}

	// Tensions-Modell without Control
	// -------------------------------

	// Model
	logit voter ctrsort2-ctrsort30 clev ctrsort2clev - ctrsort30clev [pw=wcountry]
	estimates store clevwoc

	// Predicted values
	gen Llowclevwoc   = _b[_cons] + _b[clev] if ctrsort==1
	gen Lhighclevwoc  = _b[_cons] + _b[clev]*6 if ctrsort==1
	forv i = 2/30 {
		capture replace Llowclevwoc  = _b[_cons] + _b[ctrsort`i'] + _b[clev] + _b[ctrsort`i'clev] if ctrsort==`i'
		capture replace Lhighclevwoc = _b[_cons] + _b[ctrsort`i'] + _b[clev]*6 + _b[ctrsort`i'clev]*6 if ctrsort==`i'
	}

	drop  ctrsort2clev-ctrsort30clev


	// Table
	estout lsatwc pubqualwc clevwc ///
	using anvotzuf1.tex ///
	  , replace style(tex) label ///
	  keep(men age age2 edu income occ2 occ3 occ4 occ5 emp2 emp3 emp4 emp5 group) ///
	  prehead(\begin{tabular}{lrrr} \hline  & \multicolumn{3}{c}{Ungleichheitsdimension} \\\\ ) ///
	  posthead(\hline) ///
	  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	  mlabel("Leben" "Öffentl. Dienste" "Spann.-Wahrn." ) /// 
			collabels(, none) ///
	  cells(b(fmt(%3.2f) star)) ///
	  stats(r2_p N, labels("\$r_{\text{MF}}^2\$" "\$n\$") fmt(%9.2f %9.0f)) ///
	  varlabels(_cons Konstante, ///
	  blist(occ2 "\multicolumn{4}{l}{\\emph{Klasse}} \\\\ `=char(13)'" ///
			emp2 "\multicolumn{4}{l}{\\emph{Erwerbsstatus}} \\\\ `=char(13)'" ///
			)) ///	 
	  starlevels(* 0.05) 


	// Lhat -> Phat
	// ------------

	foreach var of varlist L* {
		local stub = substr("`var'",2,.)
		gen P`stub' = exp(`var')/(1+exp(`var'))
		drop `var'
	}

	// Graphical Representation
	// -------------------------

	keep s_cntry ctrsort Plow* Phigh* 
	by ctrsort, sort: keep if _n==1

	// Reshape and Relabel
	reshape long Plow Phigh, i(ctrsort) j(var lsatwc lsatwoc pubqualwc pubqualwoc clevwc clevwoc )

	gen contrast:contrast = 1 if var == "lsatwc" | var == "lsatwoc"
	replace contrast = 2 if var == "pubqualwc" | var == "pubqualwoc"
	replace contrast = 3 if var == "clevwc" | var == "clevwoc"
	label define contrast 1 "Leben" 2 "Öffentliche Dienste"  3 "Spann.-Wahrnehmung" ///
	  , modify
	gen control = index(var,"wc")

	// Plot
	sort ctrsort
	graph twoway ///
	  (rspike Plow Phigh ctrsort if !control, horizontal blcolor(gs12) blwidth(thin)) ///
	  (scatter ctrsort Plow if !control, ms(o) mfcolor(gs12) mlcolor(gs12)) ///
	  (scatter ctrsort Phigh if !control, ms(o) mfcolor(gs12) mlcolor(gs12)) ///
	  (rspike Plow Phigh ctrsort if control, horizontal blcolor(black)) ///
	  (scatter ctrsort Plow if control, ms(O)  mfcolor(black) mlcolor(black)) ///
	  (scatter ctrsort Phigh if control, ms(O) mfcolor(white) mlcolor(black)) ///
	  ,  yscale(reverse) ylabel(1(1)15 17(1)26 28(1)30, grid gstyle(dot) valuelabel angle(horizontal)) ///
	    by(contrast, rows(1) note("")) ///
	  ytitle("")  ysize(3.7) xsize(5.6) ///
	  legend(order(5 6) lab(5 "Unzufriedene") lab(6 "Zufriedene"))
	graph export anvotzuf1a.eps, replace


	// Store some results for Later User
	// --------------------------------

	drop if !control
	gen diff = (Phigh - Plow) 
	by ctrsort, sort: egen diffmean = mean(diff)

	keep s_cntry ctrsort var diff diffmean
	reshape wide diff, i(s_cntry) j(var, string)
	ren diffmean index2
	sort s_cntry
	save anvotzuf1a, replace

	log close
	exit
	
