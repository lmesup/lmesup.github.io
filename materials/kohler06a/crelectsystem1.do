...
*	keep country* party pervote edate rilecmp
	bysort country (edate): keep if edate==edate[_N]

	// Left-Importance
	bysort country: gen leftimp = sum(pervote/100 * rilecmp)
	...
	restore
	sort iso3166_2
	merge iso3166_2 using `ess' `evs' `manifesto', nokeep
	drop _merge*

	sort iso3166_2
	order iso3166_2 s_cntry 
	save electsystem1, replace
