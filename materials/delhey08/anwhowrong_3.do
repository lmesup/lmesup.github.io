...
version 9.0
	...
	collapse (mean) correct? gdp2003 eu [aweight = weight], by(ctrname)

	gen correct0 = 0
	replace correct2 = correct1 + correct2
	replace correct3 = correct2 + correct3

	replace eu = eu*-1
	egen axis = axis(eu gdp2003), gap reverse label(ctrname)
	graph twoway                                                                ///
	  || rbar correct0 correct1 axis, horizontal fcolor(gs0) lcolor(black)      ///
	  || rbar correct1 correct2 axis, horizontal fcolor(gs8) lcolor(black) ///
	  || rbar correct2 correct3 axis, horizontal fcolor(gs16) lcolor(black)     ///
	  || , scheme(s1mono)                                             ///
	  legend(order(1 "Too low" 2 "Correct" 3 "Too high") rows(1) )    ///
	  xscale(titlegap(-3.5)) ///
	  xtitle(Percent) xlab(0 "0" .25 "25" .5 "50" .75 "75" 1 "100") ///
	  ylabel(1(1)15 17(1)26, valuelabel angle(0) ) ytitle("")
	graph export anwhowrong_3.eps, replace
	restore
