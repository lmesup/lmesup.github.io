* Evaluation of own Quality of Life on Comparison and Control-Variables
version 9
	set more off
	set scheme s1mono
	capture log close
	log using anlsat01, replace
	...
	foreach var of varlist compeco compemp compenv compsoc compqual {
		foreach ctr in AT BE CY CZ DE DK EE ES FI FR GB GR HU {
			svy: ologit ownqual `var' men age age2  ownfin2-ownfin4 edu2-edu4 empocc2-empocc8 mar2-mar5  ///
			  if iso3166_2 == `"`ctr'"'
		...
		// Table
		estout `estoutlist'  using anlsat01_`var'_part1.tex ///
		  ...
		  varlabels(_cons "Cutpoint" , ///
		    blist(                                                                                                          ///
		      ownfin2 "\multicolumn{14}{l}{\emph{Satisfaction with own financial situation (reference: very bad) }}  \\\ "  ///
		...
		// Table
		estout `estoutlist'  using anlsat01_`var'_part2.tex ///
		  , replace style(tex) label ///
		  ...
		  varlabels(_cons "Cutpoint" , ///
		    blist(                                                                                                          ///
		      ownfin2 "\multicolumn{13}{l}{\emph{Satisfaction with own financial situation (reference: very bad) }}  \\\ "  ///
	...
	graph export anlsat01.eps, replace
