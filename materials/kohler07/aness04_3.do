...
version 9.2
	...
	gen abspos = ln((l_absinc + (u_absinc - l_absinc)/2)/1000)
	label variable abspos "Log Absolute income"
	
	// "Mean Absolute Position"
	// ------------------------

	egen region = concat(region*)
	by region, sort: gen mabspos = sum(abspos)/sum(abspos!=.)
	by region, sort: replace mabspos = mabspos[_N]
	...
	by lnghoma, sort: gen mstflife = sum(mstflife)/sum(mstflife<.)
	by lnghoma: replace mstflife = mstflife[_N]
