* Analysis of the ESS 2002 - Some alternative specifications
version 9.2
	clear	
	set more off
	set memory 90m
	...
	use cntry ///
	  ...
	  health hlthhmp happy              /// Attitudes
	  polintr lrscale  vote          /// Politics
	  crmvct                     /// Crime-Victory 
	gndr yrbrn marital         /// Socio-Demographie
	  iscoco iscocop emplrel ///
	...
	mark touse
	markout touse happy men c_age c_hhmmb c_abspos c_mabspos c_relpos ia_absmean ia_relmean c_hhmmb men c_age

	reg happy men c_age c_hhmmb c_abspos [pweight=weight] if touse
	estimates store euA
	
	reg happy men c_age c_hhmmb c_abspos c_mabspos [pweight=weight] if touse
	estimates store container1
	
	reg happy men c_age c_hhmmb c_relpos [pweight=weight] if touse
	estimates store container2
	
	reg happy men c_age c_hhmmb c_mabspos [pweight=weight] if touse
	estimates store euB
	
	reg happy men c_age c_hhmmb c_abspos c_mabspos ia_absmean [pweight=weight] if touse
	estimates store ia_absmean
	
	reg happy men c_age c_hhmmb c_relpos c_mabspos ia_relmean [pweight=weight] if touse
	estimates store ia_relmean
	
	estout euA container1 euB ia_absmean ///
	using aness02_3_happy.tex ///
	...
	drop touse
	mark touse
	markout touse lrscale men c_age c_hhmmb c_abspos c_mabspos c_relpos ia_absmean ia_relmean c_hhmmb men c_age

	reg lrscale men c_age c_hhmmb c_abspos [pweight=weight] if touse
	estimates store euA
	
	reg lrscale men c_age c_hhmmb c_abspos c_mabspos [pweight=weight] if touse
	estimates store container1
	
	reg lrscale men c_age c_hhmmb c_relpos [pweight=weight] if touse
	estimates store container2
	
	reg lrscale men c_age c_hhmmb c_mabspos [pweight=weight] if touse
	estimates store euB
	
	reg lrscale men c_age c_hhmmb c_abspos c_mabspos ia_absmean [pweight=weight] if touse
	estimates store ia_absmean
	
	reg lrscale men c_age c_hhmmb c_relpos c_mabspos ia_relmean [pweight=weight] if touse
	estimates store ia_relmean
	
	estout euA container1 euB ia_absmean ///
	using aness02_3_lrscale.tex ///
	, replace style(tex) label ///
	  prehead(\begin{tabular}{lrrrr} \hline  2002 & \multicolumn{4}{c}{Model type} \\ ) ///
	  posthead(\hline) ///
	  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	...
	sum health, meanonly
	replace health = r(max) + 1 - health
	
	drop touse
	mark touse
	markout touse health men c_age c_hhmmb c_abspos c_mabspos c_relpos ia_absmean ia_relmean c_hhmmb men c_age
	ologit health men c_age c_hhmmb c_abspos [pweight=weight] if touse
	estimates store euA
	
	ologit health men c_age c_hhmmb c_abspos c_mabspos [pweight=weight] if touse
	estimates store container1
	
	ologit health men c_age c_hhmmb c_relpos [pweight=weight] if touse
	estimates store container2
	
	ologit health men c_age c_hhmmb c_mabspos [pweight=weight] if touse
	estimates store euB
	
	ologit health men c_age c_hhmmb c_abspos c_mabspos ia_absmean [pweight=weight] if touse
	estimates store ia_absmean
	
	ologit health men c_age c_hhmmb c_relpos c_mabspos ia_relmean [pweight=weight] if touse
	estimates store ia_relmean
	
	estout euA container1 euB ia_absmean ///
	using aness02_3_health.tex ///
	, replace style(tex) label ///
	  prehead(\begin{tabular}{lrrrr} \hline  2002 & \multicolumn{4}{c}{Model type} \\ ) ///
	  posthead(\hline) ///
	  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	...
	collabels(, none) ///
	  cells(b(fmt(%3.2f) star)) ///
	  drop(men c_age c_hhmmb cut1:_cons cut2:_cons cut3:_cons cut4:_cons) ///
	...
	gen voter = vote==1 if vote <= .

	// drop countries with compulsary voting
	drop if inlist(cntry,"LU","BE","GR","IT","CY","TR")

	drop touse
	mark touse
	markout touse voter men c_age c_hhmmb c_abspos c_mabspos c_relpos ia_absmean ia_relmean c_hhmmb men c_age

	logit voter men c_age c_hhmmb c_abspos [pweight=weight] if touse
	estimates store euA

	logit voter men c_age c_hhmmb c_abspos c_mabspos [pweight=weight] if touse
	estimates store container1
	
	logit voter men c_age c_hhmmb c_relpos [pweight=weight] if touse
	estimates store container2
	
	logit voter men c_age c_hhmmb c_mabspos [pweight=weight] if touse
	estimates store euB
	
	logit voter men c_age c_hhmmb c_abspos c_mabspos ia_absmean [pweight=weight] if touse
	estimates store ia_absmean
	
	logit voter men c_age c_hhmmb c_relpos c_mabspos ia_relmean [pweight=weight] if touse
	estimates store ia_relmean
	
	estout euA container1 euB ia_absmean ///
	using aness02_3_voter.tex ///
	, replace style(tex) label ///
	  prehead(\begin{tabular}{lrrrr} \hline  2002 & \multicolumn{4}{c}{Model type} \\ ) ///
	  posthead(\hline) ///
	  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	...
	collabels(, none) ///
	  cells(b(fmt(%3.2f) star)) ///
	  drop(men c_age c_hhmmb) ///
	  stats(r2_p N bic, labels("\$r^2\$" "\$n\$" "BIC") fmt(%9.2f %9.0f %9.0f)) ///
	  varlabels(_cons Constant) ///
	  starlevels(* 0.05 ** 0.01 *** 0.001) 
