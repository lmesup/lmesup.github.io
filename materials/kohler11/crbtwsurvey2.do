// Creates a harmonised dataset of election studies 1949-2009
// ----------------------------------------------------------
...
2005 WZB  "Testmodul deutsche CSES III" /// 
	  "RLD" "15+" 18Sep2005
	2009 GLES-1102 "German Longitudinal Election Study Nachwahl-Querschnitt" ///
	  "Random" "16+" 27Sep2009
end
...
// 2009 GLES-1102 "German Longitudinal Election Study"
// ------------------------------------------
// Election date was 27. September 2009 

use $gles/GLES1102_Pre1.2.dta if q2>=18, clear

gen str8 zanr = "GLES-1102"
lab var zanr "Zentralarchiv study number"

gen intstart = "28Sep2009" 
gen intend = "23Nov2009"

gen id = nw_lfdn
lab var id "Original idenifier"
isid id

gen double weight = IPFWEIGHT

gen voter:yesno = q36 == 1 if inrange(q36,1,2)
lab var voter "Voter y/n"

gen lr:lr = 1 if inlist(q24,1,2,3)
replace lr = 2 if inlist(q24,4,5)
replace lr = 3 if inlist(q24,6)
replace lr = 4 if inlist(q24,7,8)
replace lr = 5 if inlist(q24,9,10,11)
replace lr = 6 if q24==.
lab var lr "Left right self-placement"

gen polint = 5 - q75 if !inlist(q75,98,99)
lab var polint "Politicial interest"

gen party:party = 1 if q40b==22
replace party = 2 if q40b==21
replace party = 3 if q40b==23
replace party = 3 if q40b==24
replace party = 3 if inlist(q40b,25,26,27,28)
lab var party "Electoral behaviour"

gen men:yesno = q1 == 1 if inlist(q1,1,2)
lab var men "Man y/n"

gen agegroup:agegroup = 1 if inrange(q2,18,29)
replace agegroup = 2 if inrange(q2,30,64)
replace agegroup = 3 if inrange(q2,65,94)
label variable agegroup "Agegroup"

gen emp:emp = 1 if inlist(d179,1,2,4,5)
replace emp = 2 if d179 == 3 | d179==13 | d179==14
replace emp = 3 if inlist(d179,11,15) 
replace emp = 4 if inlist(d179,9,10,.) 
replace emp = 5 if inlist(d179,6,7,8) 
replace emp = 6 if emp==.
lab var emp "Employment status"

gen occ:occ = 1 if inlist(d183,2,3,8) | inlist(d198,2,3,8)
replace occ = 2 if occ == . & (inlist(d183,4,5) | inlist(d198,4,5))
replace occ = 3 if occ == . & (inlist(d183,6) | inlist(d198,6))  
replace occ = 4 if occ == .
lab var occ "Occupational status"

gen edu:edu = 1 if inlist(d176,1,2)				
replace edu = 2 if inlist(d176,3,4)
replace edu = 3 if inlist(d176,5)
lab var edu "Education"

gen bul:bul = 1 if inlist(land,1,2,4)
replace bul = 2 if land==3
replace bul = 3 if land==5
replace bul = 4 if land==6
replace bul = 5 if land==9
replace bul = 6 if land==8
replace bul = 7 if inlist(land,7,10)
replace bul = 8 if inlist(land,11,12)
replace bul = 9 if land==13
replace bul = 10 if land==14
replace bul = 11 if land==15
replace bul = 12 if land==16
lab var bul "Region"

gen mar:mar = 1 if d174 == 1 | d174 == 2
replace mar = 2 if inlist(d174,3,5,6)
replace mar = 3 if inlist(d174,4)
label variable mar "Marital status"   

xtile hhinc1 = d241a if d241a<=12000, nq(3)
xtile hhinc2 = d241b if d241b<97, nq(3)
gen hhinc = 1 if hhinc1==1 | hhinc2==1
replace hhinc = 2 if hhinc1==2 | hhinc2==2
replace hhinc = 3 if hhinc1==3 | hhinc2==3
drop hhinc1 hhinc2
label variable hhinc "Houshold income"					
label value hhinc hhinc

gen denom:denom = 1 if inlist(d244,1,2)
replace denom = 2 if inlist(d244,3)
replace denom = 3 if denom== .
label variable denom "Denomination"
