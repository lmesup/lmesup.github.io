...
// Merge resultsset of anmpred22 and election data
// -----------------------------------------------
...
// 1949 and 1953 counterfactual observed distribution of seats
egen s1949 = apport(npartyvotes) if year(eldate)==1949 ///
  , by(bsample) s(size) threshold(5) e(strpos(grundmandat,party)>0) m(jefferson)

egen s1953 = apport(npartyvotes) if year(eldate)==1953 ///
  , by(bsample) s(size) threshold(5) e(strpos(grundmandat,party)>0) m(jefferson)

replace seats = s1949 if year(eldate) == 1949
replace seats = s1953 if year(eldate) == 1953
drop s1949 s1953
...
// Gallagher index
by bsample eldate: egen S6 = sum(seats6)
...
graph export grgallagher_100.eps, replace
