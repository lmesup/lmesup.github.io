...
// ------------------------------
keep party _I* pweight eldate zanr voter bul

tempname CI
postfile `CI' bsample bul str9 zanr eldate categ Phat 	/// 
  using anmpred23_bs, replace every(1)
...
foreach k of local K {
	...
	foreach l of local L {
		...
		if !_rc {
			...
			forv j = 1/3 {
				if year(`k') <=1953 {
					levelsof bul if eldate==`k', local(M)
					foreach m of local M {
						sum Phat`j' [aw=pweight]  /// 
						  if !voter & eldate==`k' & zanr=="`l'" & bul==`m'  ///
						  , meanonly
						post `CI' (0) (`m') ("`l'") (`k') (`j') (r(mean))
					}
				}
				else {
					sum Phat`j' [aw=pweight]  /// 
					  if !voter & eldate==`k' & zanr=="`l'", meanonly
					post `CI' (0) (0) ("`l'") (`k') (`j') (r(mean))
				}
			}
			drop Phat*
		}
						...
						if year(`k') <=1953 {
							...
							foreach m of local M {
								sum Phat`j' [aw=pweight]  /// 
								  if !voter & eldate==`k' & zanr=="`l'" & bul==`m'  ///
								  , meanonly
								post `CI' (`i') (`m') ("`l'") (`k') (`j') (r(mean))
							}
						}
						else {
							sum Phat`j' [aw=pweight]  /// 
							  if !voter & eldate==`k' & zanr=="`l'", meanonly
							post `CI' (`i') (0) ("`l'") (`k') (`j') (r(mean))
						}
					}
					drop Phat*
				}
