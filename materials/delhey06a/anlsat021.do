	// Life Satisfaction on inter-national Comparisons with upward/downward interaction
	// --------------------------------------------------------------------------------

	// Reiterate with Deprivation
...
version 8
	...
	gen upward = .
	lab var upward "Upward comparison"

	gen ia = .
	lab var ia "Ref. country * upward comp."
	
	// Turkey
	foreach type in g i {
		foreach diffctry in poland hungary spain italy france sweden netherlands switzerland {
			replace selected = `diffctry'_`type'
			replace upward = selected >= 0 & selected < .
			replace ia = selected * upward
			svyreg lsat selected upward ia   men age age2 lhinceq dep edu2-edu4 emp2-emp4 occ2-occ6 mar2-mar4  ///
			  if cntry == "Turkey"
		...
		// Table
			estout poland_`type'_turkey hungary_`type'_turkey spain_`type'_turkey italy_`type'_turkey france_`type'_turkey ///
			  ...
			  using anlsat021_turkey_`type'.tex ///
		  ...
		  blist(edu2 "\multicolumn{9}{l}{\emph{Education (reference: primary) }}  \\\\" ///
	      emp2 "\multicolumn{9}{l}{\emph{Employment status (reference: full-time) }} \\\\ " ///
	      occ2   "\multicolumn{9}{l}{\emph{Occupational status (reference: unskilled worker) }} \\\\ " ///
	      mar2   "\multicolumn{9}{l}{\emph{Marital status (reference: single) }} \\\\ " ///
		  )) ///
			  starlevels(* 0.05) 
		estimates drop _all
		}
	...
	foreach type in g i {
		...
		estout poland_`type'_hungary spain_`type'_hungary italy_`type'_hungary ///
		  ...
		  sweden_`type'_hungary netherlands_`type'_hungary switzerland_`type'_hungary ///
			  using anlsat021_hungary_`type'.tex ///
		  , replace style(tex) label ///
	...
	foreach type in g i {
		...
		estout poland_`type'_germanye hungary_`type'_germanye spain_`type'_germanye italy_`type'_germanye ///
		   otherpart_`type'_germanye france_`type'_germanye  netherlands_`type'_germanye switzerland_`type'_germanye ///
			  using anlsat021_germanye_`type'.tex ///
		  , replace style(tex) label ///
		  prehead(\begin{tabular}{lrrrrrrrrr} \hline  & \multicolumn{8}{c}{Reference country} \\\\ ) ///
		  posthead(\hline) ///
		  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	...
	foreach type in g i {
		...
		estout poland_`type'_germanyw hungary_`type'_germanyw spain_`type'_germanyw italy_`type'_germanyw ///
		  otherpart_`type'_germanyw france_`type'_germanyw  netherlands_`type'_germanyw switzerland_`type'_germanyw ///
			  using anlsat021_germanyw_`type'.tex ///
		  , replace style(tex) label ///
		  prehead(\begin{tabular}{lrrrrrrrrr} \hline  & \multicolumn{8}{c}{Reference country} \\\\ ) ///
		  posthead(\hline) ///
		  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	...
	local twopt `"`twopt' legend(order(-  "Downward Comparisons:" 2 3 4) holes(1 5) cols(2) lab(2 "Upward Comparison") lab(3 "significant interaction") lab(4 "non signifcant interaction"))"' 
	...
	graph export anlsat021_i.eps, replace
	...
	graph export anlsat021_g.eps, replace
