	// Life Satisfaction on inter-national Comparisons with Income interaction
	// --------------------------------------------------------------------
...
version 8
	...
	egen iq = xtile(lhinceq), p(25(25)75) by(cntry)
	tab iq, gen(iq)
	...
	lab var selected1 "Ref.-Country (1st Income Quartile)"
	lab var selected2 "Ref.-Country * 2nd Income Quartile"
	lab var selected3 "Ref.-Country * 3rd Income Quartile"
	lab var selected4 "Ref.-Country * 4th Income Quartile"
	...
	foreach type in g i {
		foreach diffctry in poland hungary spain italy france sweden netherlands switzerland {
			...
			replace selected2 = `diffctry'_`type' * iq2
			replace selected3 = `diffctry'_`type' * iq3
			replace selected4 = `diffctry'_`type' * iq4

			svyreg lsat selected1 iq2-iq4 selected2-selected4   men age age2 dep edu2-edu4 emp2-emp4 occ2-occ6 mar2-mar4  ///
			  if cntry == "Turkey"
		...
		// Table
			estout poland_`type'_turkey hungary_`type'_turkey spain_`type'_turkey italy_`type'_turkey france_`type'_turkey ///
			  ...
			  using anlsat041_turkey_`type'.tex ///
	...
	foreach type in g i {
		...
		estout poland_`type'_hungary spain_`type'_hungary italy_`type'_hungary ///
		  ...
		  sweden_`type'_hungary netherlands_`type'_hungary switzerland_`type'_hungary ///
			  using anlsat041_hungary_`type'.tex ///
		  , replace style(tex) label ///
	...
	foreach type in g i {
		...
		estout poland_`type'_germanye hungary_`type'_germanye spain_`type'_germanye italy_`type'_germanye ///
		   otherpart_`type'_germanye france_`type'_germanye  netherlands_`type'_germanye switzerland_`type'_germanye ///
			  using anlsat041_germanye_`type'.tex ///
		  , replace style(tex) label ///
		  prehead(\begin{tabular}{lrrrrrrrrr} \hline  & \multicolumn{8}{c}{Reference Country} \\\\ ) ///
		  posthead(\hline) ///
		  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	...
	foreach type in g i {
		...
		estout poland_`type'_germanyw hungary_`type'_germanyw spain_`type'_germanyw italy_`type'_germanyw ///
		  otherpart_`type'_germanyw france_`type'_germanyw  netherlands_`type'_germanyw switzerland_`type'_germanyw ///
			  using anlsat041_germanyw_`type'.tex ///
		  , replace style(tex) label ///
		  prehead(\begin{tabular}{lrrrrrrrrr} \hline  & \multicolumn{8}{c}{Reference Country} \\\\ ) ///
		  posthead(\hline) ///
		  prefoot(\hline) postfoot(\hline \end{tabular} ) ///
	...
	local twopt `"`twopt' legend(order(-  "4th Income-Quartile:" 2 3 4) holes(1 5) cols(2) lab(2 "1st Income-Quartile") lab(3 "significant interaction") lab(4 "non signifcant interaction"))"' 
	...
	graph export anlsat041_i.eps, replace
	...
	graph export anlsat041_g.eps, replace
