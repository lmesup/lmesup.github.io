	// Reiterate anlsat01 with Deprivation
...
version 8
	...
	label def ctry 4 "Deutschland (W)" 3 "Deutschland (O)" 2 "Ungarn" 1 "Türkei"
	...
	local lineopt "clcolor(black) sort"
	
	tw ///
	  (rarea ub lb refcountry, `rareaopt') ///
	  (connected b_c refcountry, `conopt') ///
	  (line contacts own  refcountry, `lineopt' clpattern(dash shortdash)) ///
	  if type == "i"  ///
	  , by(country, rows(1) note("")) ///
	  xlab(1 "PL" 2 "HU" 3 "ES" 4 "IT" 5 "D" 6 "FR" 7 "SW" ///
	  8 "NL" 9 "CH", alternate) ///
	  xtitle("") ytitle(Regressionskoeffizienten) ///
	  legend(order(2 3 4) rows(2) lab(2 "Referenzländer") lab(3 "Nachbarn/Freunde") lab(4 "Eigenes Land")) ///
	  ylabel(0(.1).5)

	graph export anlsat011D.eps, replace
