...
graph twoway ///
 ...
 || , ylabel(`K', valuelabel angle(0) grid gstyle(dot))  ///
    ytitle("") ///
    xlabel(0(10)60) xmtick(5(10)55) ///
    xtitle("") xline(50, lpattern(dash) lcolor(black)) ///
    legend(off) ///
    graphregion(margin(l+4)) ///
    title("Germany", box bexpand) name(DE, replace) nodraw
...
graph twoway ///
 ...
 || , ylabel(`K', valuelabel angle(0) grid gstyle(dot))     ///
    ...
    title("United Kingdom", box bexpand) name(GB, replace) nodraw

graph combine DE GB, col(1) xcommon
graph export anexpol2_survey.eps, replace
...
graph combine DE GB, col(1) xcommon
graph export anexpol2_official.eps, replace
