...
graph bar absvoters pinvalid addinvalid leverage   ///
  if !inlist(iso3166,"BE","CY","LU") 	/// 
  , stack lintensity(255) 								///
  bar(1, lcolor(black) fcolor(black))  								/// 
  bar(2, lcolor(black) fcolor(gs6))  								/// 
  bar(3, lcolor(black) fcolor(gs12))  								/// 
  bar(4, lcolor(black) fcolor(white))  								/// 
  over(ctrname, sort(leverage turnout)) horizontal 	///
  legend(order(1 "Absent voters" 2 "Observed invalid ballots"  /// 
  3 "Additional Invalid ballots" 4 "Leverage") rows(2) span  ///
  size(small) symysize(*.8) symxsize(*.8))  /// 
  ysize(6.5) ylab(0(10)100)
graph export grlevbycntry1.eps, replace
...
graph twoway ///
  ...
  xscale(range(0 108)) ///
  note("Leverage = Maximum turnout - valid vote", span)
graph export grlevbycntry2.eps, replace
!epstopdf grlevbycntry2.eps
